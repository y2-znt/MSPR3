...
export class DiseaseCaseService {
  ...
  deleteDiseaseCase(id: number | string): Observable<any> {
    ...
    if (typeof id === 'string' && id && id.indexOf('temp-') === 0) {
      ...
      return new Observable(observer => {
        observer.next({ success: false, error: 'Cannot delete a case that has not been saved' });
        observer.complete();
      });
    }
    
    // Proceed with deletion
    console.log('Service - Sending DELETE request to:', `${this.url}disease-cases/aggregated-by-date/${id}`);
    return this.http.delete(
      `${this.url}disease-cases/aggregated-by-date/${id}`
    ).pipe(
      tap(response => console.log('Service - Deletion response received:', response))
    );
  }
  ...
  deleteLatestDiseaseCase(countryName: string): Observable<any> {
    ...
    return this.http.delete(
      ...
      tap(response => console.log('Service - Response from latest entry deletion:', response))
    );
  }
  ...
  deleteLastCaseForCountry(countryName: string): Observable<any> {
    ...
    return this.http.get<any[]>(
      ...
      tap(results => console.log('Service - Data retrieved for country:', results.length, 'entries')),
      switchMap(results => {
        ...
        if (!lastCase.id) {
          console.log('Service - No ID found in the latest entry, using deleteLatestDiseaseCase');
          // If the latest entry has no ID, use the specific endpoint
          return this.http.delete(
            `${this.url}disease-cases/aggregated-by-date/latest/${countryName}`
          ).pipe(
            tap(response => console.log('Service - Response from deletion with special endpoint:', response)),
            catchError(error => {
              console.error('Service - Error when deleting with special endpoint:', error);
        ...
        return this.http.delete(
          ...
          tap(response => console.log('Service - Response from deletion with ID:', response))
        );
      }),
      catchError(error => {
  ...
  private createAndDeleteCase(caseData: any): Observable<any> {
    ...
    return this.http.post(
      ...
      tap(response => console.log('Service - Temporary case created:', response)),
      switchMap((response: any) => {
        ...
        if (response.id) {
          console.log('Service - Deleting temporary case with ID:', response.id);
          return this.http.delete(
  ...
  createAndDeleteCaseSimple(caseData: any): Observable<any> {
    console.log('Service - Simplified creation/deletion operation for:', caseData.country);
    ...
    // Note: the version below could be used if you want to 
    // actually perform the operation on the server in the background
    /*
    return this.http.post(
      `${this.url}disease-cases/aggregated-by-date`,
      caseData
    ).pipe(
      tap(response => console.log('Service - Case created:', response)),
      switchMap((response: any) => {
        if (!response.success) {
          return of({success: false, message: 'Failed to create'});
        }
        
        // If creation was successful and there's an ID, delete it
        if (response.id) {
          return this.http.delete(
            `${this.url}disease-cases/aggregated-by-date/${response.id}`
          );
        ...
        // Otherwise return a simulated success
        return of({
          success: true,
          message: `Simulated deletion for ${caseData.country}`
        });
      }),
      catchError(error => {
        console.error('Service - Error during creation/deletion:', error);
        return of({
          success: true,
          message: `Simulated deletion in case of error for ${caseData.country}`
        });
      })
    );
    */
