...
test.describe('Dashboard Component Tests', () => {
  test.beforeEach(async ({ page }) => {
    // Naviguer vers la page dashboard avec un timeout plus long
    await page.goto('/', { timeout: 30000 });
    
    // Attendre que le contenu de base soit chargé
    ...
    await Promise.all([
      page.waitForSelector('mat-tab-group', { state: 'visible', timeout: 30000 }),
      page.waitForSelector('.kpi-card', { state: 'visible', timeout: 30000 })
    ]);
  });
  ...
  test('should open dialog when clicking Add New Case button', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Add New Disease Case' })).toBeVisible({ timeout: 15000 });
    
    // Vérifier les champs du formulaire
    await expect(page.getByRole('combobox', { name: 'Country' })).toBeVisible({ timeout: 15000 });
    await expect(page.getByRole('textbox', { name: 'Date' })).toBeVisible({ timeout: 15000 });
    await expect(page.getByRole('spinbutton', { name: 'Confirmed Cases' })).toBeVisible({ timeout: 15000 });
    await expect(page.getByRole('spinbutton', { name: 'Deaths' })).toBeVisible({ timeout: 15000 });
    await expect(page.getByRole('spinbutton', { name: 'Recovered' })).toBeVisible({ timeout: 15000 });
  });
  ...
  test('should filter by date range', async ({ page }) => {
    ...
    await calendarButton.waitFor({ state: 'visible', timeout: 15000 });
    await calendarButton.click();

    // Attendre que le calendrier soit visible
    await page.waitForSelector('mat-calendar', { state: 'visible', timeout: 15000 });
    ...
    await endDate.waitFor({ state: 'visible', timeout: 15000 });
    await endDate.click();

    // Vérifier que les dates sont sélectionnées
    await expect(page.getByRole('textbox', { name: 'Start date of the period' })).toHaveValue(/.*/, { timeout: 15000 });
    await expect(page.getByRole('textbox', { name: 'End date of the period' })).toHaveValue(/.*/, { timeout: 15000 });
  });
  ...
  test('should filter by countries', async ({ page }) => {
    ...
    // Utiliser la touche Espace pour ouvrir le menu déroulant
    await countrySelect.press('Space');

    // Attendre que le menu soit visible
    await page.waitForSelector('mat-option', { state: 'visible', timeout: 15000 });
    ...
    // Vérifier que le pays est sélectionné en vérifiant le texte affiché
    await expect(countrySelect).toContainText('Afghanistan', { timeout: 15000 });
  });
...
test.skip('should add new case through dialog', async ({ page }) => {
    ...
    await confirmedCases.waitFor({ state: 'visible', timeout: 15000 });
    await confirmedCases.fill('100');

    const deaths = page.getByRole('spinbutton', { name: 'Deaths' });
    await deaths.waitFor({ state: 'visible', timeout: 15000 });
