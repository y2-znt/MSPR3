...
test.describe('Dashboard Component Tests', () => {
  ...
  test('should display dashboard with all required elements', async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/Monitor and analyze/i)).toBeVisible({
      timeout: 15000,
    });
    ...
    await expect(
      page.getByRole('textbox', { name: 'Start date of the period' })
    ).toBeVisible({ timeout: 15000 });
    await expect(
      page.getByRole('textbox', { name: 'End date of the period' })
    ).toBeVisible({ timeout: 15000 });
    await expect(
    ...
    await expect(
      page.getByRole('button', { name: 'Add a new case' })
    ).toBeVisible({ timeout: 15000 });
  ...
  test('should open dialog when clicking Add New Case button', async ({
    page,
  }) => {
    ...
    await expect(dialog.getByRole('combobox', { name: 'Country' })).toBeVisible(
      { timeout: 10000 }
    );
    ...
    await expect(
      dialog.getByRole('spinbutton', { name: 'Confirmed Cases' })
    ).toBeVisible({ timeout: 10000 });
    await expect(
      dialog.getByRole('spinbutton', { name: 'Deaths' })
    ).toBeVisible({ timeout: 10000 });
    await expect(
      dialog.getByRole('spinbutton', { name: 'Recovered' })
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  //     timeout: 15000,
  //   });
  //   await expect(page.getByTestId('countries-tab')).toBeVisible({
  //     timeout: 15000,
  //   });
  ...
  test.skip('should add new case through dialog', async ({ page }) => {
    ...
    const confirmedCases = page.getByRole('spinbutton', {
    ...
    await page.waitForSelector('mat-progress-bar', {
      state: 'hidden',
      timeout: 30000,
    });
